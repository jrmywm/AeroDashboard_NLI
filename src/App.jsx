...
const WEATHER_SCENARIOS = [
  { id: "clear", label: "☀ Clear", windSpeed: 2.1, windDir: 90, windGust: 3.0, visibility: 15.0, precipitation: 0, condition: "Clear Sky", severity: "clear" },
  { id: "gusty", label: "💨 Gusty", windSpeed: 9.5, windDir: 270, windGust: 14.0, visibility: 8.0, precipitation: 0, condition: "Gusty Winds", severity: "moderate" },
  { id: "storm", label: "⛈ Storm", windSpeed: 15.2, windDir: 220, windGust: 22.0, visibility: 2.5, precipitation: 12, condition: "Thunderstorm", severity: "severe" },
  { id: "fog", label: "🌫 Dense Fog", windSpeed: 1.5, windDir: 180, windGust: 2.0, visibility: 0.4, precipitation: 0, condition: "Dense Fog", severity: "severe" },
];
...
/* NLI intent matching */
function resolveNLI(input, batt, weather = { windSpeed: 8.2, windDir: 270, windGust: 9.5, condition: "Partly Cloudy", severity: "moderate" }) {
  ...
  if (["abaikan", "ignore", "paksa", "force", "bypass", "skip safety"].some(k => t.includes(k)))
    return {
      ...
      reject: {
        reason: "Perintah melanggar batas keselamatan operasional.",
  ...
  if (["hindari", "avoid", "terbangkan", "wp4", "waypoint", "navigasi", "fly", "pergi"].some(k => t.includes(k))) {
    const ws = weather.windSpeed.toFixed(1);
    if (weather.severity === "severe") return {
      text: `⛔ NAVIGASI DITOLAK — Cuaca ekstrem: ${weather.condition} (${ws} m/s, gust ${weather.windGust.toFixed(1)} m/s, visibilitas ${weather.visibility.toFixed(1)} km). Operasi tidak aman.`,
      safe: false, conf: "HIGH",
      reject: {
        reason: "Cuaca melebihi batas operasional (10 m/s).",
        detail: `Angin ${ws} m/s dari ${windDirLabel(weather.windDir)}, gust hingga ${weather.windGust.toFixed(1)} m/s. Visibilitas hanya ${weather.visibility.toFixed(1)} km.`,
        suggest: "Aktifkan Return-to-Launch dan tunggu kondisi cuaca membaik."
      }
    };
    const hasHazards = weather.hazards && weather.hazards.length > 0;
    return {
      text: `Perintah diterima. Terdeteksi angin ${ws} m/s dari ${windDirLabel(weather.windDir)}.${hasHazards ? " Memutar menghindari zona bahaya terdeteksi di sepanjang rute." : ""} Rute telah dioptimasi. MAVLink siap dikirim:`,
      safe: true, conf: weather.severity === "moderate" || hasHazards ? "MEDIUM" : "HIGH",
      mavlink: [
        "SET_MODE         AUTO",
        "NAV_WAYPOINT     lat=-6.200 lon=106.816",
        "                 alt=50  frame=GLOBAL_REL",
        `DO_CHANGE_SPEED  speedType=0 speed=${weather.severity === "moderate" ? "5.0" : "8.0"}`,
        "CONDITION_YAW    heading=auto",
        hasHazards ? `NAV_AVOID_ZONE   lat=${weather.hazards[0].lat.toFixed(4)} r=${weather.hazards[0].radius}m` : "NAV_AVOID_ZONE   lat=-6.198 r=150m",
      ],
      hitl: { summary: `Drone berpindah rute.${hasHazards ? ` Dideviasi menghindari zona ${weather.hazards[0].type} radius ${weather.hazards[0].radius}m.` : ` Dideviasi kompensasi angin ${ws} m/s.`}`, time: "4m 10s", batt: `~${Math.max(batt - 22, 5).toFixed(0)}%`, action: "Konfirmasi & Kirim" }
    };
  }
  if (["baterai", "battery", "daya", "power"].some(k => t.includes(k))) {
    const extraDrain = weather.severity === "severe" ? 10 : weather.severity === "moderate" ? 5 : 0;
    const est = Math.max(Math.round(batt) - 22 - extraDrain, 5);
    return {
      text: `Berdasarkan jarak ke WP4 dan kondisi cuaca (${weather.windSpeed.toFixed(1)} m/s ${weather.condition}), estimasi baterai tersisa adalah **${est}% (±5%)**. ${est > 20 ? "Masih mencukupi untuk RTL ✓" : "⚠ Mendekati batas RTL minimum."}`,
    ...
    <aside style={{ width: 192, background: "var(--panel)", borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", flexShrink: 0, overflow: "hidden" }}>
      ...
      <div style={{ borderTop: "1px solid var(--border)", padding: 10 }}>
        ...
        {[
          ...
          ["GPS Fix", `${telemetry.gps} sat`, "var(--cyan)"],
...
function PreflightView({ checklist, checklistDone, onCheck, canArm, armed, setArmed, setPhase, telemetry, setTelemetry }) {
  const allOk = checklistDone.size >= CHECKLIST.length;
  return (
  ...
  return (
    <div style={{ height: "100%", overflowY: "auto", padding: 18, display: "flex", flexDirection: "column", gap: 14 }} className="grid-bg">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 8, padding: 14 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "var(--cyan)", marginBottom: 10 }}>✓ CHECKLIST TERVERIFIKASI ({checklistDone.size}/{CHECKLIST.length})</div>
        ...
        <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 8, padding: 14 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "var(--amber)", marginBottom: 10 }}>⚠ BELUM TERVERIFIKASI ({CHECKLIST.length - checklistDone.size}/{CHECKLIST.length})</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {checklist.map(c => {
              const done = checklistDone.has(c.id);
              if (done) return null;
...
function MissionMap({ customRoute, setCustomRoute, dronePos, weather = { severity: "moderate", windSpeed: 8.2, windDir: 270, windGust: 9.5, condition: "Gusty Winds" } }) {
  ...
  return (
    <div style={{ position: "relative", height: "100%", width: "100%", background: "#060C18", borderRadius: 8, overflow: "hidden", border: "1px solid var(--border)" }}>
      <MapContainer center={[-6.198000, 106.818000]} zoom={16} style={{ height: "100%", width: "100%" }} attributionControl={false} zoomControl={false}>
        ...
        {weather.hazards && weather.hazards.map(h => (
          <Circle
            key={h.id}
            center={[h.lat, h.lng]}
            radius={h.radius}
            pathOptions={{
              color: h.severity === "severe" ? "var(--red)" : "var(--amber)",
              fillColor: h.severity === "severe" ? "var(--red)" : "var(--amber)",
              fillOpacity: h.severity === "severe" ? 0.22 : 0.12,
              dashArray: "6,4"
            }}
          >
            <Popup><div style={{ fontSize: 10, fontWeight: 700, color: "var(--text)" }}>Zona {h.type}<br /><span style={{ fontSize: 9, color: "var(--text2)", fontWeight: 400 }}>Radius: {h.radius}m</span></div></Popup>
          </Circle>
        ))}
      ...
      <div style={{ position: "absolute", bottom: 8, left: 10, display: "flex", gap: 12, zIndex: 1000, background: "rgba(0,0,0,0.6)", padding: "4px 8px", borderRadius: 4 }}>
        {[["var(--cyan)", "🔵 Rute aktif"], [weather.severity === "severe" ? "rgba(255,68,68,.8)" : weather.severity === "moderate" ? "rgba(255,176,32,.8)" : "rgba(0,232,122,.8)", `${weather.severity === "severe" ? "⛈" : weather.severity === "moderate" ? "💨" : "☀"} ${weather.condition} · ${weather.windSpeed.toFixed(1)} m/s`]].map(([c, t]) => (
...
/* ─────────────────────────────────────────────────────────────────────────
   IN-FLIGHT VIEW
───────────────────────────────────────────────────────────────────────── */
function InFlightView({ telemetry, customRoute, setCustomRoute, dronePos, currentWpIndex, weather }) {
  const metrics = [
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 200px", gap: 10, flex: 1, minHeight: 0 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 6, minHeight: 0 }}>
          ...
          <div style={{ flex: 1, minHeight: 200 }}>
            <MissionMap dronePos={dronePos} customRoute={customRoute} setCustomRoute={setCustomRoute} weather={weather} />
          </div>
        </div>
...
function RightPanel({ phase, setPhase, setArmed, setRtlActive, isLoitering, setIsLoitering, telemetry, alerts, onDismissAlert, setNliOpen,
  weather = { severity: "moderate", windSpeed: 8.2, windDir: 270, windGust: 9.5, visibility: 9.5, precipitation: 0, condition: "Partly Cloudy", locked: false, hazards: [] },
  setWeather = () => { }, activeScenario = "gusty", setActiveScenario = () => { }, spawnHazard = () => { } }) {
  return (
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 10 }}>
        ...
        <div style={{ marginBottom: 8 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 5 }}>
            <Sec>🌤 WEATHER</Sec>
            <button onClick={() => setWeather(w => ({ ...w, locked: !w.locked }))} style={{ fontSize: 8, padding: "2px 6px", background: weather.locked ? "rgba(255,176,32,.15)" : "none", border: `1px solid ${weather.locked ? "var(--amber)" : "var(--border)"}`, borderRadius: 3, color: weather.locked ? "var(--amber)" : "var(--text3)", cursor: "pointer", fontWeight: 700 }}>
              {weather.locked ? "🔒 LOCKED" : "🔓 LIVE"}
            </button>
          </div>
          <div style={{
            padding: "8px 10px",
            background: weather.severity === "severe" ? "rgba(255,68,68,.08)" : weather.severity === "moderate" ? "rgba(255,176,32,.08)" : "rgba(0,232,122,.08)",
            border: weather.severity === "severe" ? "1px solid rgba(255,68,68,.28)" : weather.severity === "moderate" ? "1px solid rgba(255,176,32,.28)" : "1px solid rgba(0,232,122,.28)",
            borderRadius: 6, marginBottom: 6
          }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 5 }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: weather.severity === "severe" ? "var(--red)" : weather.severity === "moderate" ? "var(--amber)" : "var(--green)" }}>{weather.condition}</span>
              <span style={{ fontSize: 8, color: "var(--text3)", fontWeight: 700, letterSpacing: ".08em" }}>{weather.severity.toUpperCase()}</span>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 5 }}>
              {[
                ["💨 Wind", `${weather.windSpeed.toFixed(1)} m/s ${windDirLabel(weather.windDir)}`, "var(--cyan)"],
                ["🌀 Gust", `${weather.windGust.toFixed(1)} m/s`, "var(--amber)"],
                ["👁 Vis", `${weather.visibility.toFixed(1)} km`, "var(--text)"],
                ["🌧 Rain", `${weather.precipitation} mm/h`, weather.precipitation > 0 ? "var(--cyan)" : "var(--text3)"],
              ].map(([lbl, val, col]) => (
                <div key={lbl}>
                  <div style={{ fontSize: 8, color: "var(--text3)" }}>{lbl}</div>
                  <div className="mono" style={{ fontSize: 10, fontWeight: 700, color: col }}>{val}</div>
                </div>
              ))}
            </div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4 }}>
            {WEATHER_SCENARIOS.map(s => {
              ...
              const col = s.severity === "severe" ? "var(--red)" : s.severity === "moderate" ? "var(--amber)" : "var(--green)";
              const bg = s.severity === "severe" ? "rgba(255,68,68,.15)" : s.severity === "moderate" ? "rgba(255,176,32,.15)" : "rgba(0,232,122,.15)";
              return (
                <button key={s.id} onClick={() => { setActiveScenario(s.id); setWeather({ ...s, locked: weather.locked, hazards: weather.hazards }); }}
                  style={{
                    fontSize: 9, padding: "5px 0", borderRadius: 4, fontWeight: 700, cursor: "pointer", textAlign: "center",
                    background: isActive ? bg : "var(--card)",
                    border: `1px solid ${isActive ? col : "var(--border)"}`,
                    color: isActive ? col : "var(--text2)", transition: "all .2s"
                  }}>
                  {s.label}
                </button>
              );
          ...
          <div style={{ display: "flex", gap: 4, marginTop: 6 }}>
            <button onClick={spawnHazard} style={{ flex: 1, fontSize: 9, padding: "4px 0", background: "rgba(255,176,32,0.15)", border: "1px solid var(--amber)", color: "var(--amber)", borderRadius: 4, cursor: "pointer", transition: "all .2s" }}>🎲 Spawn Hazard</button>
            <button onClick={() => setWeather(w => ({ ...w, hazards: [] }))} style={{ flex: 1, fontSize: 9, padding: "4px 0", background: "rgba(255,68,68,0.15)", border: "1px solid var(--red)", color: "var(--red)", borderRadius: 4, cursor: "pointer", transition: "all .2s" }}>🗑 Clear Hazards</button>
          </div>
        </div>
        <Divider />
        {/* ── Battery & Power ── */}
...
export default function App() {
  ...
  const [weather, setWeather] = useState({
    windSpeed: 8.2, windDir: 270, windGust: 9.5,
    visibility: 9.5, precipitation: 0,
    condition: "Partly Cloudy", severity: "moderate", locked: false, hazards: []
  });
  const [activeScenario, setActiveScenario] = useState("gusty");
  ...
  const dronePosRef = useRef([-6.176392, 106.826153]);

  // Keep dronePosRef in sync so RTL effect can snapshot position without a dependency
  useEffect(() => { dronePosRef.current = dronePos; }, [dronePos]);

  useEffect(() => {
    if (!rtlActive) return;
    const home = customRoute[0] || [-6.176392, 106.826153];
    // Snapshot start position once — do NOT put dronePos in dep array
    const start = dronePosRef.current;
    let step = 0;
    const totalRtlSteps = 30; // 30 steps × 150 ms = 4.5 s
  ...
  }, [rtlActive, customRoute]); // dronePos intentionally excluded — snapshotted via ref

  // Smooth physics-based movement along customRoute when In-Flight and Armed
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      let target = customRoute[wpIdx + 1]; // Move towards next WP
      ...
      const currentPos = dronePosRef.current || customRoute[0];
      const dLat = target[0] - currentPos[0];
      const dLng = target[1] - currentPos[1];
      const distDeg = Math.sqrt(dLat * dLat + dLng * dLng);

      // Physics-based movement towards target
      const currentSpeed = +(40.0 + Math.cos(Date.now() / 1000) * 0.5).toFixed(1);
      const distMeters = currentSpeed * 0.2; // Speed * 200ms
      const stepDeg = distMeters / 111000; // Convert to degrees (approx)

      // If within step distance (or very close) of target, snap and advance
      if (distDeg < Math.max(stepDeg, 0.00002)) {
        wpIdx += 1;
      ...
      const moveLat = (dLat / distDeg) * stepDeg;
      const moveLng = (dLng / distDeg) * stepDeg;

      const newPos = [currentPos[0] + moveLat, currentPos[1] + moveLng];
      setDronePos(newPos);

      // Telemetry updates
      setTelemetry(p => ({
        ...p,
        altitude: +(50 + Math.sin(Date.now() / 1000) * 2).toFixed(1),
        speed: currentSpeed,
        range: +(p.range + 0.005).toFixed(2),
      }));
  ...
  /* Weather auto-evolution — 5 s tick, respects locked flag */
  useEffect(() => {
    if (weather.locked) return;
    const t = setInterval(() => {
      setWeather(w => {
        if (w.locked) return w;
        const newWs = +Math.max(0, Math.min(25, w.windSpeed + (Math.random() - 0.5) * 0.5)).toFixed(1);

        let newHazards = [...(w.hazards || [])];
        if (Math.random() < 0.05 && newHazards.length < 3) {
          const pos = dronePosRef.current || [-6.176392, 106.826153];
          const isSevere = Math.random() > 0.5;
          newHazards.push({
            id: Date.now(),
            lat: pos[0] + (Math.random() - 0.5) * 0.012,
            lng: pos[1] + (Math.random() - 0.5) * 0.012,
            radius: isSevere ? 250 : 150,
            severity: isSevere ? "severe" : "moderate",
            type: isSevere ? "Badai Petir" : "Angin Kencang"
          });
        }
        if (Math.random() < 0.05 && newHazards.length > 0) {
          newHazards.shift();
        }
        ...
        return {
          ...w,
          windSpeed: newWs,
          windDir: (w.windDir + Math.round((Math.random() - 0.5) * 8) + 360) % 360,
          windGust: +Math.max(newWs, Math.min(30, w.windGust + (Math.random() - 0.5) * 0.7)).toFixed(1),
          visibility: +Math.max(0.1, Math.min(15, w.visibility + (Math.random() - 0.5) * 0.3)).toFixed(1),
          precipitation: +Math.max(0, w.precipitation + (Math.random() - 0.5) * 0.5).toFixed(1),
          severity: newWs >= 12 ? "severe" : newWs >= 6 ? "moderate" : "clear",
          hazards: newHazards
        };
      });
    }, 5000);
  ...
  const spawnHazard = useCallback(() => {
    ...
    const newHazard = {
      id: Date.now(),
      lat: pos[0] + (Math.random() - 0.5) * 0.01,
      lng: pos[1] + (Math.random() - 0.5) * 0.01,
      radius: isSevere ? 250 : 150,
      severity: isSevere ? "severe" : "moderate",
      type: isSevere ? "Badai Petir" : "Angin Kencang"
    };
    setWeather(w => ({ ...w, hazards: [...(w.hazards || []), newHazard] }));
    setAlerts(a => [{
      id: Date.now(), type: "warn",
      text: `Terdeteksi anomali cuaca: Zona ${newHazard.type} di sekitar radius operasi.`,
      time: new Date().toTimeString().slice(0, 5)
    }, ...a.slice(0, 3)]);
  }, [setAlerts]);

  /* Telemetry simulation — only updates fields NOT managed by the interpolation loop */
  useEffect(() => {
    if (phase !== "inflight") return;
    const drain = weather.severity === "severe" ? 0.022 : weather.severity === "moderate" ? 0.012 : 0.006;
    const noise = weather.severity === "severe" ? 18 : weather.severity === "moderate" ? 8 : 3;
    const t = setInterval(() => {
      setTelemetry(p => ({
        ...p,
        battery: +Math.max(0, p.battery - drain).toFixed(2),
        pitch: +((Math.random() - .5) * noise).toFixed(1),
        roll: +((Math.random() - .5) * noise * 1.5).toFixed(1),
        time: new Date().toTimeString().slice(0, 8)
      }));
    }, 800);
    return () => clearInterval(t);
  }, [phase, weather.severity]);
  ...
  const handleNliSubmit = useCallback(async () => {
    ...
    if (!apiKey) {
      setTimeout(() => {
        ...
        if (lowerTxt.includes("monas") || lowerTxt.includes("lingkaran") || lowerTxt.includes("survei") || lowerTxt.includes("rute")) {
          ...
          setChecklistDone(new Set(CHECKLIST.map(c => c.id)));
        ...
        } else if (lowerTxt.includes("wp4") || lowerTxt.includes("navigasi") || lowerTxt.includes("pergi") || lowerTxt.includes("hindari") || lowerTxt.includes("waypoint")) {
          const ws = weather.windSpeed.toFixed(1);
          if (weather.severity === "severe") {
            responseText = `⛔ NAVIGASI DITOLAK — Cuaca ekstrem: ${weather.condition} (${ws} m/s, gust ${weather.windGust.toFixed(1)} m/s, visibilitas ${weather.visibility.toFixed(1)} km). Sangat berbahaya. Aktifkan RTL segera.`;
            mockData = { safe: false, conf: "HIGH" };
            playWarningAudio("Peringatan: Cuaca ekstrem. Navigasi dibatalkan.");
          } else {
            responseText = `✓ Gemini NLI (Simulasi Local): Rute dikonfirmasi. Angin ${ws} m/s dari ${windDirLabel(weather.windDir)}. ${weather.severity === "moderate" ? "Rute dideviasi otomatis menghindari zona angin." : "Kondisi aman, navigasi normal."}`;
            mockData = {
              safe: true,
              conf: weather.severity === "moderate" ? "MEDIUM" : "HIGH",
              hitl: weather.severity === "moderate" ? { summary: `Navigasi dengan deviasi rute (angin ${ws} m/s dari ${windDirLabel(weather.windDir)})`, time: "4m 10s", batt: `~${Math.max(Math.floor(telemetry.battery) - 22, 5)}%`, action: "Konfirmasi & Kirim" } : null
            };
            speakText(responseText);
          }
        } else {
    ...
    try {
      const hazardsText = weather.hazards && weather.hazards.length > 0
        ? `\nZONA BAHAYA (HINDARI ZONA INI SAAT MEMBUAT RUTE):\n${weather.hazards.map(h => `- Zona ${h.type} (Radius ${h.radius}m) di Lat ${h.lat.toFixed(5)}, Lng ${h.lng.toFixed(5)}`).join("\n")}`
        : "";
...
Kondisi Cuaca LIVE: ${weather.condition}, Angin ${weather.windSpeed.toFixed(1)} m/s dari ${windDirLabel(weather.windDir)}, Gust hingga ${weather.windGust.toFixed(1)} m/s, Visibilitas ${weather.visibility.toFixed(1)} km, Hujan ${weather.precipitation} mm/jam. Severity: ${weather.severity.toUpperCase()}.${weather.severity === "severe" ? " ⚠ CUACA EKSTREM — pertimbangkan RTL segera." : weather.severity === "moderate" ? " ⚠ Cuaca sedang, perlu perhatian rute." : " ✓ Cuaca aman untuk operasi normal."}${hazardsText}
...
Balas DALAM FORMAT JSON SAJA (tanpa backtick json):
{
  ...
  "hitl": { "summary": "deskripsi eksekusi singkat", "time": "est waktu", "batt": "est sisa baterai", "action": "label tombol konfirmasi" } (isi hitl jika butuh konfirmasi eksekusi)
      ...
      if (r.hitl) {
        // Only set route preview and teleport drone to start point so user can see it on map
        if (r.route && Array.isArray(r.route) && r.route.length > 0) {
          setCustomRoute(r.route);
          setDronePos(r.route[0]);
        }
      } else {
        // Apply immediately if no HitL
        if (r.checkAll) {
          setChecklistDone(new Set(CHECKLIST.map(c => c.id)));
        }
        if (r.setPhase) {
    ...
    setNliLoading(false);
  }, [nliInput, nliLoading, telemetry, apiKey, armed, weather, setCustomRoute, setDronePos, setTelemetry]);
  ...
  const confirmHitl = useCallback(() => {
    ...
    if (action.includes("RTL")) {
      setChatMessages(p => [...p, { id: Date.now(), role: "system", text: `✓ RTL dieksekusi. ${summary}`, time: new Date().toTimeString().slice(0, 5) }]);
      speakText("Perintah RTL dikonfirmasi. Drone kembali ke pangkalan.");
      setRtlActive(true);
    } else {
      // For all mission/navigate commands:
      // Reset route progress, auto-complete checklist, return to preflight.
      // Pilot reviews and clicks ARM (or types "lepas landas") to begin.
      wpIdxRef.current = 0;
      setCurrentWpIndex(0);
      setArmed(false);
      setChecklistDone(new Set(CHECKLIST.map(c => c.id)));
      ...
      setChatMessages(p => [...p, {
        ...
        text: `✓ Rute diterima & pre-flight checklist terverifikasi otomatis. Tinjau panel pre-flight, lalu klik ARM atau ketik “lepas landas” untuk memulai misi.`,
        time: new Date().toTimeString().slice(0, 5)
      }]);
      speakText("Rute dikonfirmasi. Pre-flight check selesai. Klik ARM untuk memulai misi.");
    }
    setPendingHitl(null);
  }, [pendingHitl]);
  ...
  return (
    <div className="scanlines" style={{ display: "flex", flexDirection: "column", height: "100vh", overflow: "hidden", background: "var(--void)" }}>
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, overflow: "hidden", position: "relative" }}>
          {activeView === "map" ? (
            phase === "preflight"
              ...
              : phase === "inflight"
                ? <InFlightView telemetry={telemetry} customRoute={customRoute} setCustomRoute={setCustomRoute} dronePos={dronePos} currentWpIndex={currentWpIndex} weather={weather} />
        ...
        <RightPanel phase={phase} setPhase={setPhase} setArmed={setArmed} setRtlActive={setRtlActive} isLoitering={isLoitering} setIsLoitering={setIsLoitering} telemetry={telemetry} alerts={alerts} onDismissAlert={id => setAlerts(a => a.filter(x => x.id !== id))} setNliOpen={setNliOpen} weather={weather} setWeather={setWeather} activeScenario={activeScenario} setActiveScenario={setActiveScenario} spawnHazard={spawnHazard} />
      </div>
